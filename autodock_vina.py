...
def vina_dock(receptor_file, ligand_file, map_profile):
  ...
  v.write_pose('outputs/' + str(creation_time)+ '/' + str(receptor_file) + '_' + str(ligand_file) + '_vina_minimized.pdbqt', overwrite=True)

  # Dock the ligand
  v.dock(exhaustiveness=32, n_poses=20)
  v.write_poses('outputs/' + str(creation_time)+ '/' + str(receptor_file) + '_' + str(ligand_file) + '_vina.pdbqt', n_poses=5, overwrite=True)
